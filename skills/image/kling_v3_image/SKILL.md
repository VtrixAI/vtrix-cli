...
name: kling_v3_image
description: >-
  Use the `kirin_v3_image` model via the Vtrix CLI for general image generation, editing, subject-consistent reference work, and wider multi-image creative control.
...
```bash
vtrix models spec kirin_v3_image --output json
```

Build the `vtrix run kirin_v3_image ...` command from the current JSON spec every time.
...
If `vtrix models spec kirin_v3_image --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `vtrix models spec kirin_v3_image --output json`
