...
name: wan2.7-image
description: >-
  Use the `wan27_image` model via the Vtrix CLI for general image generation, fast visual iteration, image editing, and multi-image reference work.
...
```bash
vtrix models spec wan27_image --output json
```

Build the `vtrix run wan27_image ...` command from the current JSON spec every time.
...
If `vtrix models spec wan27_image --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `vtrix models spec wan27_image --output json`
