...
name: gpt_image_1
description: >-
  Use the `gpt_image_1` model via the Vtrix CLI for broad OpenAI image generation, prompt-led visual ideation, and flexible image workflow coverage.
...
```bash
vtrix models spec gpt_image_1 --output json
```

Build the `vtrix run gpt_image_1 ...` command from the current JSON spec every time.
...
If `vtrix models spec gpt_image_1 --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `vtrix models spec gpt_image_1 --output json`
