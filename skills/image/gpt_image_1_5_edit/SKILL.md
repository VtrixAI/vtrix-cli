...
description: >-
  Use the `gpt_image_1_5_edit` model via the Vtrix CLI for refined single-image editing, stronger text replacement, and cleaner presentation-ready revisions.
...
```bash
vtrix models spec gpt_image_1_5_edit --output json
```

Build the `vtrix run gpt_image_1_5_edit ...` command from the current JSON spec every time.
...
If `vtrix models spec gpt_image_1_5_edit --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `vtrix models spec gpt_image_1_5_edit --output json`
