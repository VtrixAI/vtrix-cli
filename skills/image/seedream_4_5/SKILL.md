...
name: seedream_4_5
description: >-
  Use the `spark_dream_4_5` model via the Vtrix CLI for premium Seedream image generation with stronger consistency, richer detail, and higher creative control.
...
```bash
vtrix models spec spark_dream_4_5 --output json
```

Build the `vtrix run spark_dream_4_5 ...` command from the current JSON spec every time.
...
If `vtrix models spec spark_dream_4_5 --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `vtrix models spec spark_dream_4_5 --output json`
