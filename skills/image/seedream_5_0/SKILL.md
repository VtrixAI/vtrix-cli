...
name: seedream_5_0
description: >-
  Use the `spark_dream_5_0` model via the Vtrix CLI for flagship Seedream image generation with the highest-end detail, polish, and premium visual finish in this family.
...
```bash
vtrix models spec spark_dream_5_0 --output json
```

Build the `vtrix run spark_dream_5_0 ...` command from the current JSON spec every time.
...
If `vtrix models spec spark_dream_5_0 --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `vtrix models spec spark_dream_5_0 --output json`
