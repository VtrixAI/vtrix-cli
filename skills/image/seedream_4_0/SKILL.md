...
name: seedream_4_0
description: >-
  Use the `spark_dream_4_0` model via the Vtrix CLI for balanced Seedream image generation with better lighting, detail, and prompt understanding than the lightest baseline.
...
```bash
vtrix models spec spark_dream_4_0 --output json
```

Build the `vtrix run spark_dream_4_0 ...` command from the current JSON spec every time.
...
If `vtrix models spec spark_dream_4_0 --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `vtrix models spec spark_dream_4_0 --output json`
