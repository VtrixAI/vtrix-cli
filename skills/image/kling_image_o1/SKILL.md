...
name: kling_image_o1
description: >-
  Use the `kirin_image_omni` model via the Vtrix CLI for premium multi-input Kling image generation and editing with stronger detail and refined visual finish.
...
```bash
vtrix models spec kirin_image_omni --output json
```

Build the `vtrix run kirin_image_omni ...` command from the current JSON spec every time.
...
If `vtrix models spec kirin_image_omni --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `vtrix models spec kirin_image_omni --output json`
