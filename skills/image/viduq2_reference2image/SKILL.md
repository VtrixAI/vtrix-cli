...
description: >-
  Use the `viduq2_reference2image` model via the Vtrix CLI for reference-guided image generation with stronger style and subject consistency.
...
```bash
vtrix models spec viduq2_reference2image --output json
```

Build the `vtrix run viduq2_reference2image ...` command from the current JSON spec every time.
...
If `vtrix models spec viduq2_reference2image --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `vtrix models spec viduq2_reference2image --output json`
