...
description: >-
  Use the `gpt_image_1_edit` model via the Vtrix CLI for broad single-image editing, controlled revisions, text replacement, and practical visual cleanup.
...
```bash
vtrix models spec gpt_image_1_edit --output json
```

Build the `vtrix run gpt_image_1_edit ...` command from the current JSON spec every time.
...
If `vtrix models spec gpt_image_1_edit --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `vtrix models spec gpt_image_1_edit --output json`
