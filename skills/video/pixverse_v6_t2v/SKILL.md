...
description: >-
  Use the `pixverse_v6_t2v` model via the Vtrix CLI for PixVerse prompt-only video generation with cinematic motion, longer outputs, native audio, and high-resolution delivery.
...
```bash
vtrix models spec pixverse_v6_t2v --output json
```

Build the `vtrix run pixverse_v6_t2v ...` command from the current JSON spec every time.
...
If `vtrix models spec pixverse_v6_t2v --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `vtrix models spec pixverse_v6_t2v --output json`
