...
description: >-
  Use the `pixverse_v6_transition` model via the Vtrix CLI for PixVerse first-frame to last-frame transition video generation with cinematic motion and native audio support.
...
```bash
vtrix models spec pixverse_v6_transition --output json
```

Build the `vtrix run pixverse_v6_transition ...` command from the current JSON spec every time.
...
If `vtrix models spec pixverse_v6_transition --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `vtrix models spec pixverse_v6_transition --output json`
