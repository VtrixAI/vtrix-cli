...
description: >-
  Use the `viduq2_pro_extend` model via the Vtrix CLI for premium Vidu video extension and continuation generation.
...
```bash
vtrix models spec viduq2_pro_extend --output json
```

Build the `vtrix run viduq2_pro_extend ...` command from the current JSON spec every time.
...
If `vtrix models spec viduq2_pro_extend --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `vtrix models spec viduq2_pro_extend --output json`
