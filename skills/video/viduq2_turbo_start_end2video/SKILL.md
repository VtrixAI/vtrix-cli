...
description: >-
  Use the `viduq2_turbo_start_end2video` model via the Vtrix CLI for fast start-frame and end-frame guided Vidu video generation.
...
```bash
vtrix models spec viduq2_turbo_start_end2video --output json
```

Build the `vtrix run viduq2_turbo_start_end2video ...` command from the current JSON spec every time.
...
If `vtrix models spec viduq2_turbo_start_end2video --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `vtrix models spec viduq2_turbo_start_end2video --output json`
