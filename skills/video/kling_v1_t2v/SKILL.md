...
name: kling_v1_t2v
description: >-
  Use the `kirin_v1_t2v` model via the Vtrix CLI for lightweight prompt-only Kling video generation and simple motion ideas.
...
```bash
vtrix models spec kirin_v1_t2v --output json
```

Build the `vtrix run kirin_v1_t2v ...` command from the current JSON spec every time.
...
If `vtrix models spec kirin_v1_t2v --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `vtrix models spec kirin_v1_t2v --output json`
