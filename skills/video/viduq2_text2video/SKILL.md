...
description: >-
  Use the `viduq2_text2video` model via the Vtrix CLI for balanced Vidu text-to-video generation with a strong quality-speed tradeoff and broad resolution support.
...
```bash
vtrix models spec viduq2_text2video --output json
```

Build the `vtrix run viduq2_text2video ...` command from the current JSON spec every time.
...
If `vtrix models spec viduq2_text2video --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `vtrix models spec viduq2_text2video --output json`
