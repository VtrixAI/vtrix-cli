...
description: >-
  Use the `pixverse_upload_image` model via the Vtrix CLI for the PixVerse upload step that converts a source image into a reusable `img_id` for downstream PixVerse video workflows.
...
```bash
vtrix models spec pixverse_upload_image --output json
```

Build the `vtrix run pixverse_upload_image ...` command from the current JSON spec every time.
...
If `vtrix models spec pixverse_upload_image --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `vtrix models spec pixverse_upload_image --output json`
