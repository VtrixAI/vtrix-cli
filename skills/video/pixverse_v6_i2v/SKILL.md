...
description: >-
  Use the `pixverse_v6_i2v` model via the Vtrix CLI for PixVerse image-to-video generation with cinematic camera motion, native audio, and up to 1080p output.
...
```bash
vtrix models spec pixverse_v6_i2v --output json
```

Build the `vtrix run pixverse_v6_i2v ...` command from the current JSON spec every time.
...
If `vtrix models spec pixverse_v6_i2v --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `vtrix models spec pixverse_v6_i2v --output json`
