...
description: >-
  Use the `kirin_v2_1_master_t2v` model via the Vtrix CLI for master-tier Kling text-to-video generation with stronger polish and premium finish.
...
```bash
vtrix models spec kirin_v2_1_master_t2v --output json
```

Build the `vtrix run kirin_v2_1_master_t2v ...` command from the current JSON spec every time.
...
If `vtrix models spec kirin_v2_1_master_t2v --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `vtrix models spec kirin_v2_1_master_t2v --output json`
