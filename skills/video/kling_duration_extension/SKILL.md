...
description: >-
  Use the `kirin_duration_extension` model via the Vtrix CLI for extending an existing Kling clip instead of starting a new one from scratch.
...
```bash
vtrix models spec kirin_duration_extension --output json
```

Build the `vtrix run kirin_duration_extension ...` command from the current JSON spec every time.
...
If `vtrix models spec kirin_duration_extension --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `vtrix models spec kirin_duration_extension --output json`
