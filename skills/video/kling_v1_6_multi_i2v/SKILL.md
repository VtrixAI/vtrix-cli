...
description: >-
  Use the `kirin_v1_6_multi_i2v` model via the Vtrix CLI for multi-image Kling video generation with subject consistency across several reference images.
...
```bash
vtrix models spec kirin_v1_6_multi_i2v --output json
```

Build the `vtrix run kirin_v1_6_multi_i2v ...` command from the current JSON spec every time.
...
If `vtrix models spec kirin_v1_6_multi_i2v --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `vtrix models spec kirin_v1_6_multi_i2v --output json`
