...
description: >-
  Use the `viduq3_turbo_text2video` model via the Vtrix CLI for faster Vidu Q3 text-to-video generation with audio-visual output and newer generation quality.
...
```bash
vtrix models spec viduq3_turbo_text2video --output json
```

Build the `vtrix run viduq3_turbo_text2video ...` command from the current JSON spec every time.
...
If `vtrix models spec viduq3_turbo_text2video --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `vtrix models spec viduq3_turbo_text2video --output json`
