...
name: kling_v2_6_t2v
description: >-
  Use the `kirin_v2_6_t2v` model via the Vtrix CLI for advanced Kling text-to-video generation with smoother transitions and stronger handling of more complex scenes.
...
```bash
vtrix models spec kirin_v2_6_t2v --output json
```

Build the `vtrix run kirin_v2_6_t2v ...` command from the current JSON spec every time.
...
If `vtrix models spec kirin_v2_6_t2v --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `vtrix models spec kirin_v2_6_t2v --output json`
