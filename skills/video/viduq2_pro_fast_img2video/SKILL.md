...
description: >-
  Use the `viduq2_pro_fast_img2video` model via the Vtrix CLI for fast Vidu image-to-video generation with stronger value and better quality than the lightest entry tier.
...
```bash
vtrix models spec viduq2_pro_fast_img2video --output json
```

Build the `vtrix run viduq2_pro_fast_img2video ...` command from the current JSON spec every time.
...
If `vtrix models spec viduq2_pro_fast_img2video --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `vtrix models spec viduq2_pro_fast_img2video --output json`
