...
name: seedance_2_0
description: >-
  Use the `spark_dance_v2_0` model via the Vtrix CLI for cinematic multimodal video generation across text, image, video, and audio inputs.
...
Build the `vtrix run spark_dance_v2_0 ...` command from the current JSON spec every time.
...
If `vtrix models spec spark_dance_v2_0 --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `vtrix models spec spark_dance_v2_0 --output json`
