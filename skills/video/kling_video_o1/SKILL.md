...
name: kling_video_o1
description: >-
  Use the `kirin_video_o1` model via the Vtrix CLI for broad multimodal Kling video generation across text, image, video, and custom element inputs.
...
```bash
vtrix models spec kirin_video_o1 --output json
```

Build the `vtrix run kirin_video_o1 ...` command from the current JSON spec every time.
...
If `vtrix models spec kirin_video_o1 --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `vtrix models spec kirin_video_o1 --output json`
