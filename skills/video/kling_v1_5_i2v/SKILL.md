...
name: kling_v1_5_i2v
description: >-
  Use the `kirin_v1_5_i2v` model via the Vtrix CLI for polished V1-era Kling image-to-video generation from a single source image.
...
```bash
vtrix models spec kirin_v1_5_i2v --output json
```

Build the `vtrix run kirin_v1_5_i2v ...` command from the current JSON spec every time.
...
If `vtrix models spec kirin_v1_5_i2v --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `vtrix models spec kirin_v1_5_i2v --output json`
