...
description: >-
  Use the `kirin_v3_omni_video` model via the Vtrix CLI for flagship multimodal Kling video generation across text, image, video, and custom element inputs.
...
```bash
vtrix models spec kirin_v3_omni_video --output json
```

Build the `vtrix run kirin_v3_omni_video ...` command from the current JSON spec every time.
...
If `vtrix models spec kirin_v3_omni_video --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `vtrix models spec kirin_v3_omni_video --output json`
