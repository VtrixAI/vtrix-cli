...
name: kling_v2_1_i2v
description: >-
  Use the `kirin_v2_1_i2v` model via the Vtrix CLI for refined Kling image-to-video generation with better motion control and stronger stability than older baseline variants.
...
```bash
vtrix models spec kirin_v2_1_i2v --output json
```

Build the `vtrix run kirin_v2_1_i2v ...` command from the current JSON spec every time.
...
If `vtrix models spec kirin_v2_1_i2v --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `vtrix models spec kirin_v2_1_i2v --output json`
