...
description: >-
  Use the `viduq1_text2video` model via the Vtrix CLI for cost-conscious or rapid-prototype text-to-video generation in the Vidu family.
...
```bash
vtrix models spec viduq1_text2video --output json
```

Build the `vtrix run viduq1_text2video ...` command from the current JSON spec every time.
...
If `vtrix models spec viduq1_text2video --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `vtrix models spec viduq1_text2video --output json`
