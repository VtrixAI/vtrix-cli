...
description: >-
  Use the `vidu20_img2video` model via the Vtrix CLI for classic Vidu image-to-video generation with improved motion and better detail retention than older entry models.
...
```bash
vtrix models spec vidu20_img2video --output json
```

Build the `vtrix run vidu20_img2video ...` command from the current JSON spec every time.
...
If `vtrix models spec vidu20_img2video --output json` fails, retry once. If it still fails, tell the user the spec service is temporarily unavailable and stop rather than guessing unsupported fields.
...
1. `vtrix models spec vidu20_img2video --output json`
